...
```
```{r linear plots, include = TRUE, fig.cap= "Plots of predictors vs log odds to incpect for assumption of linerarity", fig.width=6, fig.height=3, fig.align='center'}
...
```
```{r dist_plots, include = TRUE, fig.cap= "Frequency distribution to understand distribution of predictors by response class", fig.width=6, fig.height=3, fig.align='center'}
...
```{r cooks plot, include = TRUE, fig.cap= "Cooks distance to inspect for overly influential observations", fig.width=6, fig.height=3, fig.align='center'}
...
```{r threshold assesment, include = TRUE, fig.cap= "Positive predictive power as a function of probability threshold", fig.width=6, fig.height=3, fig.align='center'}
...
***
## Question 2
The complete linkage and single linkage clustering algorithms work in a similar way in as much as they start with all cases in their own cluster and work their way to a point where all cases are in a single cluster. They do this by joining the two cases / clusters which are closest as judged by the distance metric used. The difference between single and complete linkage falls in the way the distance that the newly formed cluster in each step is calculated. In single linkage the distance is taken to be the distance from each point outside the cluster to the closest case/ cluster inside the cluster. In complete linkage this distance is calculated as the most distant point inside the cluster. The working and dendrograms for both single and complete linkage clustering using the given dissimilarity matrix are shown in the below figures.
...
```{r ac values, include = TRUE, fig.cap= "Summary of model perfromance using agglomerative coefficient", fig.width=6, fig.height=3, fig.align='center'}
...
```{r swc and gap, include = TRUE, fig.cap= "Assesment of cluster quality vs  clusters using Silhoutte width criteria and Gap Statistic", fig.width=6, fig.height=3, fig.align='center'}
...
```
It can be seen from the clustering that the hypothesis proposed is very likely correct as clustering can very accurately predict which class the cancer belongs to. Further the clustering suggest the presence of sub-types that roughly join at a level of similarity as the AML cluster, if these are actually relevant is a domain specific question.
...
```{r Appendix, echo = TRUE, include = TRUE, eval=FALSE}
##################
### Question 1 ###
##################
